...
def get_db_connection(max_retries=3, retry_delay=5):
    ...
    server = "nba-database.cdwc0cs2228h.us-east-2.rds.amazonaws.com"
    database = "NBA_Database"
    username = "admin"
    password = "Alexander1985!"
    conn_str = f"DRIVER={{ODBC Driver 17 for SQL Server}};SERVER={server};DATABASE={database};UID={username};PWD={password};Connection Timeout=300"
    ...
    for attempt in range(max_retries):
        try:
            ...
            conn.timeout = 600
            # Set lock timeout to 10 minutes
            conn.execute("SET LOCK_TIMEOUT 600000")
            # Set helpful settings
            conn.execute("SET ARITHABORT ON")
            conn.execute("SET TEXTSIZE 2147483647")
            log_message(f"Connected to database successfully on attempt {attempt + 1}")
            return conn
        except Exception as e:
            if attempt < max_retries - 1:
                log_message(f"Connection attempt {attempt + 1} failed: {str(e)}")
